...
static pco_map_entry pco_camera_map[] = {
    ...
    {CAMERATYPE_PCO4000, 14, 0.0000090, 0.0000090, TRUE},
    {CAMERATYPE_PCO_DIMAX_STD, 12, 0.0000110, 0.0000110, TRUE},
    {0, 0, 0.0, 0.0, FALSE},
};
...
static gint get_max_timeout_millis(UcaPcoMe4CameraPrivate *priv)
{
...
static gpointer grab_func(gpointer rawptr)
{
    ...
    {
        int index;
        err = pco_next_image_index_ex(priv->pco, &index, get_max_timeout_millis(priv));
        if (index <= 0 || err != 0)
            return NULL;
