...
static gboolean check_and_resize_memory(UcaPcoClhsCameraPrivate *priv, GError **error)
{
    ...
    void* sts;
    Fg_getParameter(priv->fg, FG_CAMSTATUS, &sts, priv->fg_mem);
...
static gboolean uca_pco_clhs_camera_grab(UcaCamera *camera, gpointer data, GError **error)
{
    ...
    data = frame;
    memcpy((guint16*) data, frame, priv->buffer_size);
...
static void uca_pco_clhs_camera_class_init(UcaPcoClhsCameraClass *klass)
{
    ...
    pco_clhs_properties[PROP_GRAB_N_BUFFERS] =
        g_param_spec_uint("image-buffer-count",
                          "Number of image buffers",
                          "Set the number of images that will be grabbed (software trigger)",
                          1, UINT32_MAX, 1,
                          G_PARAM_READWRITE);
